...
def plot_counting(steigung=1.0, y_achsenabschnitt=0.0):
    ...
    ax.plot(x_vals, y_vals, color='royalblue', label=f'line: y = {steigung:.1f} * (x-150) + {y_achsenabschnitt:.2f}')

    # Bereich einfärben
    
    ax.fill_between(x_vals, y_vals, 92, where=(y_vals < 92), color='navajowhite', alpha=1, label='categorized as sick')
    ax.fill_between(x_vals, y_vals, 62, color='lightblue', alpha=1, label='categorized as healthy')
    ...
    st.table([["falsly categorized healthy persons",f"{total_distance_cats:.2f}"],["falsly categorized sick persons",f"{total_distance_dogs:.2f}"]])
...
with col1:
    ...
    y_achsenabschnitt = SyncedSlider('y-intercept', 50, 110, random.randint(50, 110), key_prefix="slider_y", step=1)
