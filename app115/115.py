...
def plot_with_new_function(w1=0.3, b1=17.0, w2=0.3, b2=17.0):
    ...
    ax.plot(x_vals, y_vals, '-', color='royalblue', label=f'sum of the neurons')
    ...
    st.table([["**loss**", f"**{total_distance_cats+total_distance_dogs:.2f}**"]])
...
with col1:
    st.write("#")
    st.write("###")
    steigung1 = SyncedSlider("weight 1", -2.0, 2.0, round(random.uniform(-2.0, 2.0),1), key_prefix="slider_s1", step=0.1)
    y_achsenabschnitt1 = SyncedSlider('bias 1', -50, 50, random.randint(-50, 50), key_prefix="slider_y1", step=1)
    #st.divider()
    steigung2 = SyncedSlider("weight 2", -2.0, 2.0, round(random.uniform(-2.0, 2.0),1), key_prefix="slider_s2", step=0.1)
