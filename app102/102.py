...
def plot_counting(y_achsenabschnitt=0.0):
    ...
    ax[0].plot(x_vals, y_vals, color='royalblue', label=f'boundary at {y_achsenabschnitt:.2f} kg')

    # Bereich einfärben
    ax[0].fill_between(x_vals, y_vals, 12, color='lightblue', alpha=1, label='categorized as a cat')
    ax[0].fill_between(x_vals, y_vals, 42, where=(y_vals < 42), color='navajowhite', alpha=1, label='categorized as a dog')

    ax[0].set_xlim(10, 68)
    ax[0].set_ylim(12, 42)
    ax[0].set_xlabel('size (cm)')
    ...
    ax[1].set_xlabel('weight boundary (kg)')
...
with col1:
    y_achsenabschnitt = SyncedSlider("weight boundary (kg)", 10, 50, random.randint(10, 50), key_prefix="slider_y", step=1)
